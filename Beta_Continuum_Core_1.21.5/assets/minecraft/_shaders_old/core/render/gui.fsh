...
void main() {
    ...
    if (tooltip == 1) {
        ...
        switch ((edge.x<<0)+(edge.y<<1)+(edge.z<<2)+(edge.w<<3)) {
            case 1: //left, 0001
                i = (corner.x)           + (side.y)             * sizes.z; color = tooltcol; break;
            case 2: //top, 0010
                i = (side.x)             + (corner.y)           * sizes.x; color = tooltcol; break;
            case 4: //right, 0100
                i = (sizes.z-corner.z-1) + (side.y)             * sizes.z; color = tooltcol; break;
            case 8: //bottom, 1000
                i = (side.x)             + (sizes.z-corner.w-1) * sizes.x; color = tooltcol; break;
            case 3: //topleft, 0011
                i = (corner.x)           + (corner.y)           * sizes.z; color = tooltcol; break;
            case 6: //topright, 0110
                i = (sizes.z-corner.z-1) + (corner.y)           * sizes.z; color = tooltcol; break;
            case 9: //botleft, 1001
                i = (corner.x)           + (sizes.z-corner.w-1) * sizes.z; color = tooltcol; break;
            case 12: //botright, 1100
                i = (sizes.z-corner.z-1) + (sizes.z-corner.w-1) * sizes.z; color = tooltcol; break;
            default:
                //color = hex(base);
